...
# ---------------------------------------------------------
# 3. 문서 생성 엔진 (VMP, Recipe, Protocol)
# ---------------------------------------------------------
...
def generate_master_recipe_excel(method_name, target_conc, unit, stock_conc, req_vol, sample_type, powder_info=""):
    ...
    ws.merge_range('A1:F1', f'Material Planner: {method_name}', title_fmt)
    ws.write('A3', "Sample Type:", header); ws.write('B3', sample_type, cell)
    ws.write('A4', "Stock Conc:", header); ws.write('B4', stock_conc, num)
    ws.write('A5', "Target Conc:", header); ws.write('B5', target_conc, num)
...
def generate_protocol_premium(method_name, category, params, stock_conc=None, req_vol=None, target_conc_override=None):
    ...
    doc.add_heading('1. 목적', level=1); doc.add_paragraph("본 시험법의 적합성을 검증한다.")
    doc.add_heading('2. 기기 및 시약', level=1)
    doc.add_paragraph(f"기기: {params.get('Instrument', 'N/A')}\n컬럼: {params.get('Column_Plate', 'N/A')}")
    doc.add_heading('3. 시험 방법', level=1)
    doc.add_paragraph("상세 내용은 SOP를 참조한다.")
    doc_io = io.BytesIO(); doc.save(doc_io); doc_io.seek(0)
    return doc_io

# ---------------------------------------------------------
# 4. 엑셀 스마트 일지 생성 (Final Corrected Logic)
# ---------------------------------------------------------
def generate_smart_excel(method_name, category, params):
    ...
    ws1.write(9, 1, target_conc_val, calc)
    ...
    for i in range(1, 7):
        ws_sst.write(i+1, 0, i, cell); ws_sst.write_row(i+1, 1, ["", "", "", "", ""], calc)
    ...
    ws_sst.write('A14', "※ Acceptance Criteria:", crit_fmt); ws_sst.write('A15', "1) RSD of RT & Area ≤ 2.0%"); ws_sst.write('A16', "2) Tailing Factor (1st Inj) ≤ 2.0")

    # 4.3 Specificity Sheet
    ...
    ws_spec.write('A10', "Criteria: Interference ≤ 0.5% of Std Area", crit_fmt)
    ...
    ws2.write(row+4, 0, "※ Acceptance Criteria:", crit_fmt); ws2.write(row+5, 0, "1) R² ≥ 0.990"); ws2.write(row+6, 0, "2) %RSD ≤ 5.0%")
...
# ---------------------------------------------------------
# 7. 메인 UI (Streamlit Loop - Clean Version)
# ---------------------------------------------------------
